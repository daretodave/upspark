...
export class Logger {
    ...
    public static push(error: boolean, plain: boolean, ...args: any[]): ILogger {
        ...
        this._log.messages.unshift(message);
        if (this._log.messages.length > this.max) {
            this._log.messages.splice(this.max);
        }
    ...
    public static log(error: boolean = false, ...args: any[]): ILogger {
        return error ?
